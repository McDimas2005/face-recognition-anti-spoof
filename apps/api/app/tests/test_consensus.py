...
def test_consensus_store_respects_custom_window():
    store = TemporalConsensusStore()
    old = datetime.now(UTC) - timedelta(seconds=3)
    new = datetime.now(UTC)
    ...
    frames = store.add(
        "session-1",
        "client-1",
        CandidateFrame(timestamp=new, person_id="person-1", similarity=0.9, second_score=0.1),
        window_seconds=2,
    )
